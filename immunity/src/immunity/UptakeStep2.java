...
//		COMPENSATORY NEW ORGANELLE
	...
	private static void newSecretion(Cell cell, String selectedRab) {
		...
		System.out.println("secretion " +	InitialOrganelle.getInstance().getInitOrgProp().get("kind9"));
		HashMap<String, Double> initOrgProp = new HashMap<String, Double>(
				InitialOrganelle.getInstance().getInitOrgProp().get("kind9"));
...
//		System.out.println(mem + ModelProperties.getInstance().getSecretionRate().get(mem) + "   secretion 1111  " + valueInER+membraneContent);
	...
	private static void newUptake(Cell cell, String selectedRab) {
		double cellLimit = 3d * Cell.orgScale;
		System.out.println("UPTAKE INITIAL ORGANELLES " +	InitialOrganelle.getInstance().getInitOrgProp().get("kind1"));
		HashMap<String, Double> initOrgProp = new HashMap<String, Double>(
				InitialOrganelle.getInstance().getInitOrgProp().get("kind1"));
...
//		}
	...
	//			UPTAKE ENDOSOME JUST RABA
		...
		for (String mem : membraneMet){
			...
			//			 FOR UPTAKE LOADING IN NEW ENDOSOMES
			if (InitialOrganelle.getInstance().getInitMembraneContent().get("kind1").
					containsKey(mem)) 
			{ 
				valueInEn =InitialOrganelle.getInstance().getInitMembraneContent().get("kind1").get(mem)*area; 
			} 
...
//			System.out.println(mem + valueInTotal + "   UPTAKE FROM PM 1111  " + valueInPM+membraneContent);
		...
		for (String sol : solubleMet){
			...
			// FOR UPTAKE LOADING IN NEW ENDOSOMES
			if (InitialOrganelle.getInstance().getInitSolubleContent().get("kind1").containsKey(sol)) 
			{ 
				valueInEn =InitialOrganelle.getInstance().getInitSolubleContent().get("kind1").get(sol)*volume; 
			}
			valueInTotal = valueInEn + valueInPM; 
...
//					System.out.println(membraneContent + " " + solubleContent + " " + rabContent+" " + initOrgProp);
		...
		System.out.println(kind + " UPTAKE INITIAL ORGANELLES " +	InitialOrganelle.getInstance().getInitOrgProp().get(kind));

		HashMap<String, Double> initOrgProp = new HashMap<String, Double>(
				InitialOrganelle.getInstance().getInitOrgProp().get(kind));
