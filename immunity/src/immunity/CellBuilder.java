...
//			context.add(ModelProperties);	
		...
		InitialOrganelle initialOrganelle  = InitialOrganelle.getInstance();
		context.add(initialOrganelle);		
		// Microtubules

		for (int i = 0; i < (int) 10/Cell.orgScale; i++) {// 10 MT standard
			context.add(new MTs(space, grid));
		}
...
//		
		...
		Set<String> diffOrganelles = initialOrganelle.getDiffOrganelles();
		String name;
		int ite=0;
		...
		// total surface.  These values were obtained of simulations that progressed by 40000 steps
			for (String kind : diffOrganelles){
					...
					HashMap<String, Double> initOrgProp =  new HashMap<String, Double>(initialOrganelle.getInitOrgProp().get(kind));
					...
					while (totalArea > 32d*Math.PI*minRadius*minRadius){
						...
						HashMap<String, Double> rabKindContent = new HashMap<String, Double>(initialOrganelle.getInitRabContent().get(kind));
						...
						HashMap<String, Double> rabContent = new HashMap<String, Double>(initialOrganelle.getInitRabContent().get(kind));
						for (String rab : rabContent.keySet()){
							double rr = rabContent.get(rab);
							rabContent.put(rab, rr*area);
						}
						HashMap<String, Double> membraneContent = new HashMap<String, Double>(initialOrganelle.getInitMembraneContent().get(kind));
						for (String mem : membraneContent.keySet()){
							if (mem.equals("membraneMarker")){
								membraneContent.put(mem, 1d);
								initialOrganelle.getInitMembraneContent().get(kind).remove("membraneMarker");
							}
						...
						HashMap<String, Double> solubleContent = new HashMap<String, Double>(initialOrganelle.getInitSolubleContent().get(kind));
						for (String sol : solubleContent.keySet()){
							if (sol.equals("solubleMarker")){
								solubleContent.put(sol, 1d);
								initialOrganelle.getInitSolubleContent().get(kind).remove("solubleMarker");
							}
...
//			System.out.println("FREEZE DRY METHOD   "+diffOrganelles);
			for (String kind : diffOrganelles){
				...
				HashMap<String, Double> initOrgProp =  new HashMap<String, Double>(InitialOrganelle.getInstance().getInitOrgProp().get(kind));
				HashMap<String, Double> rabContent = new HashMap<String, Double>(InitialOrganelle.getInstance().getInitRabContent().get(kind));
				HashMap<String, Double> membraneContent = new HashMap<String, Double>(InitialOrganelle.getInstance().getInitMembraneContent().get(kind));
				HashMap<String, Double> solubleContent = new HashMap<String, Double>(InitialOrganelle.getInstance().getInitSolubleContent().get(kind));
...
//				System.out.println ("SCALE SCALE "+Scale.getScale500nm()/2d);
			...
			if (obj instanceof MTs) {
				((MTs) obj).changePosition((MTs)obj);
			} 
...
//					}
		...
		if (RunEnvironment.getInstance().isBatch()) {
			RunEnvironment.getInstance().endAt(120010);
		}
