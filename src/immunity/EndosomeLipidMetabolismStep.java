...
//
		...
		for (String met : metabolites) {
			if (endosome.membraneContent.containsKey(met)) {
				...
				antigenPresentation.setInitialConcentration(met, Math.round(metValue*1E9d)/1E9d);
				...
				antigenPresentation.setInitialConcentration(met, Math.round(metValue*1E12d)/1E12d);
				localM.put(met, metValue);
		...
		antigenPresentation.setInitialConcentration("protonCy", 1e-04);
		localM.put("protonCy", 1e-04);
//		antigenPresentation.setInitialConcentration("preP", 1e-04);
//		localM.put("preP", 1e-04);
		if (localM.get("proton")==null||localM.get("proton") < 1e-05){
			antigenPresentation.setInitialConcentration("proton", 1e-04);
...
//		CModel model = antigenPresentation.getModel();
		...
		for (int time = 0; time < stepNro; time = time + 1){
			...
			for (int met = 1; met < metNro +1; met = met +1){
				...
				endosome.getAntigenTimeSeries().put((int) (tick+time*Cell.timeScale/0.03),value);
			}
		}
