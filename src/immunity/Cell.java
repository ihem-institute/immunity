...
import repast.simphony.util.ContextUtils;
   ...
   public class Cell {
 	...
 	/*public List<Endosome> pickEndosome(){
		List<Endosome> endosome = new ArrayList<Endosome>();
		for (Object obj : grid.getObjects()) {
			if (obj instanceof Endosome) {
				endosome.add((Endosome) obj);
				return endosome;
				}
			}
		return endosome;
 	...
 	public void uptake(){
 		if (tMembrane < sEndo || rabCell.get("RabA")< sEndo) return;
 		 /*Endosome budEnd = new Endosome(this.space, this.grid, null, null, null);
 		  ...
 		  context.add(budEnd); 
			...
			solubleContent.put("dextran",4d / 3d * Math.PI * 30d * 30d * 30d );
	 		Context<Object> context = ContextUtils.getContext(this);
	 		//Endosome budEndo = new Endosome(this.space, this.grid, null, null, null);
	 		//context.add(budEndo);
